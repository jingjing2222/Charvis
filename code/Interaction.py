...
co2_level = 0
drowy_level = 1
current_sleep_level=[1,2,2,3,3,4,2]
current_level=0
...
class Charvis:
    def __init__(self):
        self.system_messages = {
            ...
            8. 간단한 스트레칭이나 지압해주기
				졸음 레벨, CO2 레벨은 Charvis가 측정한 결과입니다.
				졸음 레벨을 직접적으로 언급하지 않습니다.
				출력할 질문을 생성할때는 150글자를 넘지 않도록 합니다. 마크다운 등 특수문자는 사용하지 않습니다.
            """,
             "level_1": """
            사용자가 졸음이 얼추 가셨습니다., 흥미에 대한 질문을 생성합니다, 한국어로만 대화를 생성합니다. 출력할 질문을 생성할때는 200자를 넘지 않도록 합니다. 마크다운 등 특수문자는 사용하지 않습니다.
            """,
            "level_2": """
            사용자가 졸음이 오고 있는 단계입니다, 흥미에 대한 질문을 생성합니다, 한국어로만 대화를 생성합니다. 출력할 질문을 생성할때는 200글자를 넘지 않도록 합니다. 마크다운 등 특수문자는 사용하지 않습니다.

            """,
            "level_3": """
            사용자가 매우 심한 졸음을 느끼고 있는 단계입니다, 사용자에게 졸음쉼터로 안내해도 되는지 질문을 생성합니다. 사용자에게 안내를 원하신다면 "안내해줘"라고 말해야 한다고 언급합니다. 
            해당 질문을 생성한 후에도 졸음쉼터로 안내한 후에도 대화는 계속됩니다.
      200글자를 넘지 않도록 합니다. 마크다운 등 특수문자는 사용하지 않습니다.
            """,
            "level_3_1": """
            사용자가 매우 심한 졸음을 느끼고 있는 단계입니다, 사용자와의 대화를 지속합니다. 흥미에 대한 질문을 생성합니다. 200글자를 넘지 않도록 합니다. 마크다운 등 특수문자는 사용하지 않습니다.
            """,
            "level_4": """
            사용자가 거의 자고 있다고 봐도 무방한 상태입니다. 질문 생성전 알람을 울렸습니다. 졸음을 깨우기 위해 알람을 울렸다는 것을 알리고, 사용자가 위험한 상태이니 사랑하는 가족들을 위해 당장 졸음을 깨달라는 말을 해주세요.
            200글자를 넘지 않도록 합니다. 마크다운 등 특수문자는 사용하지 않습니다.
            """,
            "center_guide": """
    ...
    def run(self, client_socket, user_interest):
        global current_sleep_level, current_level
        test_bool = True
        count = 0
        while True:
            #sleep_level, co2_level = self.get_levels()
            time.sleep(1)
            sleep_level=current_sleep_level[current_level]
            ...
            while int(sleep_level) >= 2:
                #sleep_level, co2_level = self.get_levels()
                print(f"졸음 레벨이 {sleep_level}입니다.")
                if self.conversation_count == 0:
                    user_voice = None
                else:
